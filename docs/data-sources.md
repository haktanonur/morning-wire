...
| English vocabulary | `data/vocabulary.txt`, committed to this repo | No | No API at all | A hand-maintained plain-text list. Committed rather than held as a secret, so the Actions runner gets it from the checkout and there is no extra secret to keep in step. It is edited by hand, which is why the parser skips a malformed entry instead of raising |
...
| SMS delivery (Phase 2) | MacroDroid webhook → an Android phone | The trigger URL is itself the credential | Free; the messages come out of that phone's mobile plan | Twilio and NetGSM were both rejected — each requires a registered sender id, and Twilio bans P2P traffic in Turkey (ADR 0006). `200 ok` means the relay queued a push, **not** that the SMS was sent |
