...
| English vocabulary | `data/vocabulary.txt`, gitignored | No | No API at all | A hand-maintained plain-text list. It is personal, so it stays out of the repo and reaches the runner as the `VOCABULARY_TXT_GZ` secret; `data/vocabulary.example.txt` shows the format. It is edited by hand, which is why the parser skips a malformed entry instead of raising |
